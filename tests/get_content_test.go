...
// TestGet_Text_FormElements tests that GetText correctly returns
...
func TestGet_Text_FormElements(t *testing.T) {
	b := newBrowser(t)
...
// TestGet_HTML_FormElements tests that GetHTML returns meaningful
// content for void/form elements (where innerHTML is always empty).
func TestGet_HTML_FormElements(t *testing.T) {
	b := newBrowser(t)
	openPage(t, b, "phase9_get_test.html")
	snap := mustSnapshot(t, b)
	t.Logf("Snapshot:\n%s", snap.Text)
...
// TestGet_Value_FormElements tests that GetValue works on all form elements.
func TestGet_Value_FormElements(t *testing.T) {
	b := newBrowser(t)
	openPage(t, b, "phase9_get_test.html")
	snap := mustSnapshot(t, b)
	t.Logf("Snapshot:\n%s", snap.Text)
...
// TestGet_Text_NonInteractiveElements tests GetText on elements
// that are not interactive (heading, paragraph, link, status div).
func TestGet_Text_NonInteractiveElements(t *testing.T) {
	b := newBrowser(t)
	openPage(t, b, "phase9_get_test.html")
	snap := mustSnapshot(t, b)
	t.Logf("Snapshot:\n%s", snap.Text)
...
// TestGet_HTML_NonInteractiveElements tests GetHTML on regular elements
// and void elements like img.
func TestGet_HTML_NonInteractiveElements(t *testing.T) {
	b := newBrowser(t)
	openPage(t, b, "phase9_get_test.html")
	snap := mustSnapshot(t, b)
	t.Logf("Snapshot:\n%s", snap.Text)
...
// TestGet_AfterDOMMutation tests that get commands still work
// after the DOM has been mutated (stale backend node IDs scenario).
func TestGet_AfterDOMMutation(t *testing.T) {
	b := newBrowser(t)
	openPage(t, b, "phase9_get_test.html")
