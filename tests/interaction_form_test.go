...
// TestForm_Submission validates the full form workflow:
// Fill -> KeyboardType -> Type -> Select -> Check -> Press -> Click submit -> verify.
func TestForm_Submission(t *testing.T) {
	b := newBrowser(t)
...
// TestInteraction_HoverAndDoubleClick validates hover and double-click interactions.
func TestInteraction_HoverAndDoubleClick(t *testing.T) {
	b := newBrowser(t)
	openPage(t, b, "phase1_interaction.html")
	snap := mustSnapshot(t, b)
...
// TestScroll_Basic validates scroll and scroll-into-view.
...
// TestCheckbox_CheckAndUncheck validates check/uncheck toggle.
func TestCheckbox_CheckAndUncheck(t *testing.T) {
	b := newBrowser(t)
	openPage(t, b, "phase1_interaction.html")
	snap := mustSnapshot(t, b)
